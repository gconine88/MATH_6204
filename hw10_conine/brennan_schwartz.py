...
def solve(A, d, ex):
    ...
    x = solve_body(a, b, c, dfloat, exfloat)
...
@njit
def solve_body(a, b, c, dfloat, exfloat):
    ''' Brennan-Schwartz algorithm to solve a tridiagonal system of equations
    ...
    g: numpy array
        the vector of early-excercise values
    ...
    for i in range(1, n, 1):
        ...
        beta[i] = dfloat[i] - (beta[i-1] * c[i-1]) / alpha[i-1]
    
    #initialize and set last step
    x = np.zeros(n)
    x[n-1] = np.maximum(exfloat[n-1], beta[n-1] / alpha[n-1])
    
    # second (backwards) loop to find solutions
    for j in range(n-2, -1, -1): #indices are weird, want to step from n-2 to 0 
        x[j] = np.maximum(exfloat[j], (beta[j] - b[j] * x[j+1]) / alpha[j])
